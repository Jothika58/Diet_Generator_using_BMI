        ...
        <table>
            <tr>
                <td>
					...
					{
						$Q="select uid from userinfo where email='".$_POST["txtEmail"]."' and pwd='".$_POST["txtPwd"]."'";
						$res=mysql_query($Q)or die(mysql_error());
						if($row=mysql_fetch_array($res))
						{
							
							$_SESSION['user']=$_POST['txtEmail'];
								echo "<script>window.top.location='welcomeUser.php'</script>";exit;	
								//header('location:welcomeUser.php');
						}
						else
						{
							echo'<font color="red">invalid credential</font>';
						}
				   }
				?>
							</td>
